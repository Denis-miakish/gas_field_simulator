...
class DCS:
    ...
    def __init__(self, p_line: float, cr: float, q_ext: float):
        """
        :param p_line: Давление в магистрали (константа), атм 
        :param cr: Степень сжатия (Compression Ratio) 
        :param q_ext: Дебит стороннего газа, поступающего на манифолд, ст.м3/сут
        """
        self.p_line = p_line
        self.cr = cr
        self.q_ext = q_ext
    ...
    def get_state(self, q_cluster_std: float) -> NodeState:
        ...
        # Согласно ТЗ, для ДКС поля q_res, v, rho могут быть None
        return NodeState(
            p_in=p_in,
            p_out=self.p_line,
            q_std=q_total,
